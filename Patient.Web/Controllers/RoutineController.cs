...
namespace Patient.Web.Controllers
{
    ...
    public class RoutineController : ControllerBase
    {
        ...
        [HttpGet("InitiateAppointments")]
        public async Task InitiateAppointments()
        {
            try
            {
                var staffs = await _context.Staffs.Where(m => m.DateVerified.Value.Date == DateTime.Now.Date.AddDays(-1)).ToListAsync();
